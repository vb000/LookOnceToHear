...
from src.datasets.multi_ch_simulator import (
    CIPICSimulator, APLSimulator, RRBRIRSimulator, ASHSimulator, CATTRIRSimulator, PRASimulator)
...
class MixLibriSpeechCIPICDataset(Dataset):
    def __init__(self, fg_dir, bg_dir, embed_dir, jams_dir, hrtf_list, dset,
                 sr=None, resample_rate=None, num_enroll=10, enroll_len=5, hrtf_type="CIPIC") -> None:
        super().__init__()
        ...
        if hrtf_type == 'CIPIC':
            ...
            self.multi_ch_simulator = CATTRIRSimulator(self.hrtf_list, sr)
    ...
    def __getitem__(self, idx):
        ...
        multi_ch_events, multi_ch_noise = self.multi_ch_simulator.simulate(
            event_audio_list[1:], event_audio_list[0], seed=multi_ch_seed)
