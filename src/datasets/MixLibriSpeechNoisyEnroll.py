...
class MixLibriSpeechNoisyEnroll(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Scale and add white, pink and brown noise
        multi_ch_noise = multi_ch_noise * self.noise_scale
        ...
        # Scale noise and add white, pink and brown noise
        enroll_event_audio_list[0] = enroll_event_audio_list[0] * self.noise_scale
        if self.augment and self.dset == 'train':
            white_noise = \
                generate_white_noise(multi_ch_noise.shape, self.max_white_noise_level).float()
            pink_noise = \
